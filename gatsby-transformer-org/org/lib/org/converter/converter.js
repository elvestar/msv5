...
Converter.prototype = {
  ...
  convertNode: function (node, recordHeader, insideCodeElement) {
    ...
    case Node.types.header:
      ...
      if (re.exec(childText) !== null) {
        ...
        if (timeStr.length === 18) {
           eventDate = new Date(timeStr.substr(0, 10) + ' ' + timeStr.substr(timeStr.length - 5)) 
           eventAnchor = timeStr.substr(0, 10).replace(/-/g, '') + '_' + timeStr.substr(timeStr.length - 5).replace(/:/g, '')
        } else {
           eventDate = new Date(timeStr.substr(0, 10)) 
           eventAnchor = timeStr.substr(0, 10).replace(/-/g, '')
        }
        newEvent = {
            title: eventTitle,
            date: eventDate,
    ...
    case Node.types.link:
      // console.log(node)
      if (newEvent != undefined && newEvent.cover === '') {
        if (/^\.\.\/static\/.+.jpg$/.exec(node.src)) {
            newEvent.cover = node.src
            newEvent.cover = newEvent.cover.replace('../static/', '/static/')
            fields = newEvent.cover.split('/')
            fields.splice(fields.length - 1, 0, 'small')
            newEvent.cover = fields.join('/')
            if (newEvent.summary === '') {
              newEvent.summary = 'nil'
            }
        }
      }
  ...
  convertNodesInternal: function (nodes, recordHeader, insideCodeElement) {
    ...
    for (var i = 0; i < nodes.length; ++i) {
      ...
      if (newEvent != undefined && node.type !== Node.types.header) {
        newEvent['body'] += nodeText;
      }
    }
  ...
  convertNodes: function (nodes, recordHeader, insideCodeElement) {
    events = []
    var result = this.convertNodesInternal(nodes, recordHeader, insideCodeElement);
    if (newEvent != undefined) {
      events.push(newEvent);
      newEvent = undefined;
    }
