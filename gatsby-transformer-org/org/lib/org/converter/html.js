...
ConverterHTML.prototype = {
  ...
  convert: function () {
    ...
    var toc = this.computeToc(2);
    var tocHTML = this.tocToHTML(toc);
    
    var tocHTMLNew = '<div class="tocInline"><h2>目录</h2>' + _.replace(tocHTML, /nav nav-toc/g, "nav-toc") + "</div>";
    var pos = contentHTML.indexOf('<h2 id=');
    if (pos !== -1) {
      contentHTML = [contentHTML.slice(0, pos), tocHTMLNew, contentHTML.slice(pos)].join("");
    }
    ...
    function tocToHTMLFunction(tocList) {
      ...
      for (var i = 0; i < tocList.length; ++i) {
        ...
        var headerLink = this.inlineTag("a", sectionNumber + header, {
          href: "#" + sectionNumberText.replace(/\./g, "-")
        });
  ...
  convertHeader: function (node, childText, auxData,
    ...
    if (sectionNumberText) {
      ...
      headerAttributes["id"] = "" + sectionNumberText.replace(/\./g, "-");
    }
  ...
  convertLink: function (node, childText, auxData) {
    ...
    if (this.imageExtensionPattern.exec(srcParameterStripped)) {
      ...
      smallImgSrc = fields.join('/')
      smallImgPath = `./content${smallImgSrc}`
      var dimensions = null;
      if (fs.existsSync(smallImgPath)) {
        dimensions = imageSize(smallImgPath);
      ...
      // console.log('new src: '+ smallImgSrc)
      ...
      var imgTag = null;
      if (dimensions === null) {
        imgTag = this.inlineTag("img", null, {
          src: smallImgSrc,
          alt: imgTitle,
          title: imgTitle
        }, auxData);
      ...
      return this.inlineTag("a", imgTag, {
        class: 'fancybox',
        href: imgSrc,
        title: imgTitle,
        rel: imgSrc
